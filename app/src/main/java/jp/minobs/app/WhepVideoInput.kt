...
class WhepVideoInput(private val context: Context) {
  ...
  fun createFilter(endpoint: String, bearerToken: String? = null, onStatus: (String) -> Unit = {}): SurfaceFilterRender {
    stop()
    stopped.set(false)
    return SurfaceFilterRender { texture ->
      ...
      start(endpoint.trim(), bearerToken?.trim().orEmpty(), surface, onStatus)
    }
  }

  private fun start(endpoint: String, token: String, surface: Surface, onStatus: (String) -> Unit) {
    if (!endpoint.startsWith("http://", true) && !endpoint.startsWith("https://", true)) {
      onStatus("WHEP URL must be http(s)://")
      return
    }
    Thread({
      try {
        ...
        val observer = object : PeerConnection.Observer {
          ...
          override fun onIceConnectionChange(state: PeerConnection.IceConnectionState?) { onStatus("WHEP ICE ${state?.name}") }
          override fun onIceConnectionReceivingChange(receiving: Boolean) = Unit
          override fun onIceGatheringChange(state: PeerConnection.IceGatheringState?) { if (state == PeerConnection.IceGatheringState.COMPLETE) iceComplete.countDown() }
          ...
          override fun onAddTrack(receiver: RtpReceiver?, mediaStreams: Array<out MediaStream>?) {
            ...
            attachTrack(video, surface, onStatus)
          }
          override fun onConnectionChange(newState: PeerConnection.PeerConnectionState?) { onStatus("WHEP ${newState?.name}") }
        }
        val pc = f.createPeerConnection(config, observer) ?: error("PeerConnection failed")
        peer = pc
        pc.addTransceiver(MediaStreamTrack.MediaType.MEDIA_TYPE_VIDEO).direction = RtpTransceiver.RtpTransceiverDirection.RECV_ONLY
        ...
        if (!offerLatch.await(5, TimeUnit.SECONDS)) error("Offer timeout")
        val created = offer ?: error(offerError ?: "Offer failed")
        ...
        if (!stopped.get()) onStatus("WHEP error: ${e.message ?: e.javaClass.simpleName}")
      }
    }, "MiniOBS-WHEP").start()
  }

  private fun attachTrack(video: VideoTrack, surface: Surface, onStatus: (String) -> Unit) {
    if (stopped.get()) return
    synchronized(this) {
      track?.removeSink(renderer)
      renderer?.release()
      val egl = eglBase ?: return
      val sink = EglRenderer("MiniOBS-WHEP")
      sink.init(egl.eglBaseContext, EglBase.CONFIG_PLAIN, GlRectDrawer())
      sink.createEglSurface(surface)
      video.addSink(sink)
      track = video
      renderer = sink
      onStatus("WHEP video receiving")
    }
  }
  ...
  private fun postOffer(endpoint: String, token: String, sdp: String): String {
    ...
    if (body.isBlank()) error("Empty WHEP SDP answer")
    return body
  }
